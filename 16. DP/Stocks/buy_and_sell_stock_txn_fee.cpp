// Input:
// prices = [1,3,2,8,4,9], fee = 2
// Output:
// 8
// Explanation:
// The maximum profit can be achieved by:
// Buying at prices[0] = 1, Selling at prices[3] = 8
// Buying at prices[4] = 4, Selling at prices[5] = 9
// The total profit is ((8 - 1) - 2) + ((9 - 4) - 2) = 8.

// Recursion
int findMax(int ind,int buy,int n,int fee,vector<int>& prices)
{
    if(ind==n)
        return 0;
    int profit=0;
    if(buy)
    {
        int pick=-prices[ind]+findMax(ind+1,0,n,fee,prices);
        int notPick=findMax(ind+1,1,n,fee,prices);
        profit=max(pick,notPick);
    }
    else
    {
        int pick=prices[ind]-fee+findMax(ind+1,1,n,fee,prices);
        int notPick=findMax(ind+1,0,n,fee,prices);
        profit=max(pick,notPick);
    }
    return profit;
}

int maxProfit(vector<int>& prices,int fee)
{
    int n=prices.size();
    return findMax(0,1,n,fee,prices);
}

// Memoization
int findMax(int ind,int buy,int n,int fee,vector<int>& prices,vector<vector<int>>& dp)
{
    if(ind==n)
        return 0;
    int profit=0;
    if(dp[ind][buy]!=-1)
        return dp[ind][buy];
    if(buy)
    {
        int pick=-prices[ind]+findMax(ind+1,0,n,fee,prices,dp);
        int notPick=findMax(ind+1,1,n,fee,prices,dp);
        profit=max(pick,notPick);
    }
    else
    {
        int pick=prices[ind]-fee+findMax(ind+1,1,n,fee,prices,dp);
        int notPick=findMax(ind+1,0,n,fee,prices,dp);
        profit=max(pick,notPick);
    }
    return dp[ind][buy]=profit;
}

int maxProfit(vector<int>& prices,int fee)
{
    int n=prices.size();
    vector<vector<int>> dp(n+1,vector<int>(2,-1));
    return findMax(0,1,n,fee,prices,dp);
}

// Tabulation
int maxProfit(vector<int>& prices,int fee)
{
    int n=prices.size();
    vector<vector<int>> dp(n+1,vector<int>(2,0));
    for(int i=n-1;i>=0;i--)
    {
        for(int j=1;j>=0;j--)
        {
            int profit=0;
            if(j)
            {
                int pick=-prices[i]+dp[i+1][0];
                int notPick=dp[i+1][1];
                profit=max(pick,notPick);
            }
            else
            {
                int pick=prices[i]-fee+dp[i+1][1];
                int notPick=dp[i+1][0];
                profit=max(pick,notPick);
            }
            dp[i][j]=profit;
        }
    }
    return dp[0][1];
}

// Space Optimization
int maxProfit(vector<int>& prices,int fee)
{
    int n=prices.size();
    vector<int> next(2,0);
    for(int i=n-1;i>=0;i--)
    {
        vector<int> curr(2,0);
        for(int j=1;j>=0;j--)
        {
            int profit=0;
            if(j)
            {
                int pick=-prices[i]+next[0];
                int notPick=next[1];
                profit=max(pick,notPick);
            }
            else
            {
                int pick=prices[i]-fee+next[1];
                int notPick=next[0];
                profit=max(pick,notPick);
            }
            curr[j]=profit;
        }
        next=curr;
    }
    return next[1];
}
